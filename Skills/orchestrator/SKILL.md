...
| **gumloop-workflow-designer** | Maps/designs Gumloop workflows | Existing workflow OR requirements | Workflow specification document |
...
AUTOMATION LAYER (domain-agnostic tools)
└── gumloop-workflow-designer (automates any process)
```
...
F) I need to automate a process
```
...
- F → gumloop-workflow-designer (automation design)
